...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('trade_alerts_dev.log'),  # Separate log file for dev
...
app = FastAPI(
    title="Trade Alerts SMS Parser - DEVELOPMENT",
    description="A lean SMS-based trade alerting system - DEVELOPMENT VERSION",
    version="1.0.0-dev"
)
...
DEV_MODE = True
DEV_PORT = 8001
DEV_DATABASE = "market_states_dev.db"
DEV_LOG_FILE = "trade_alerts_dev.log"
...
async def root():
    return {
        "message": "Trade Alerts SMS Parser - DEVELOPMENT VERSION", 
        "status": "healthy",
        "mode": "development",
        "port": DEV_PORT,
...
async def receive_sms(request: Request):
    ...
    try:
        ...
        logger.info(f"[DEV] Received SMS from {sender}: {message}")
...
**TIME:** {display_time}"""
        ...
        if response.status_code == 204:
            logger.info("[DEV] Discord alert sent successfully")
        else:
...
if __name__ == "__main__":
    ...
    # Development-specific startup
    logger.info("=" * 60)
    logger.info("STARTING DEVELOPMENT VERSION")
    logger.info(f"Port: {DEV_PORT}")
    logger.info(f"Database: {DEV_DATABASE}")
    logger.info(f"Log File: {DEV_LOG_FILE}")
    logger.info("=" * 60)
    
    # Use development port, fallback to environment variable
    port = int(os.environ.get("PORT", DEV_PORT))
